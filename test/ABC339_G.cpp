...
int main()
{
  ...
  inp_arr(A);
  vpii compressor(N);
  rep(i, N) compressor[i] = {A[i], i};
  sort(compressor);
  vi ind(N);
  rep(i, N) ind[compressor[i].second] = i;

  ps_segtree ps_seg(N + 1, vl(N, 0));
  rep(i, N) ps_seg.set(ind[i], (ll)A[i]);
  ...
  while (Q--)
  {
    ...
    int i = lower_bound(all(compressor), pii(X, N)) - compressor.begin();
    B = ps_seg.prod(R, 0, i) - ps_seg.prod(L, 0, i);
