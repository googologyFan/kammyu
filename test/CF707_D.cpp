...
int main()
{
  ...
  {
    if (b.score == inf)
      return T{a.bs, a.bs.count() + b.bs.count()};
    else
      return T{a.bs, a.score + b.score};
  };
  auto e = []()
  { return T{0, 0}; };
  using ps_segtree = kammyu::persistent_segtree<T, op, e>;

  vector<T> vec(N, T{0, inf});
  ps_segtree bookshelf(Q, vec);
  vb reved(N, false);
  int rev_cnt = 0;
  ...
  while (Q--)
  {
    // rep(i, N)
    // {
    //   rep(j, M) cerr << bookshelf.get(i).bs[j];
    //   cerr << " ";
    // }
    // cerr << endl;
    int q;
    ...
    if (q == 1)
    {
      ...
      i--;
      j--;
      T shelf = bookshelf.get(i);
      shelf.bs[j] = true;
      bookshelf.set(i, shelf);
    }
    if (q == 2)
    {
      int i, j;
    ...
    if (q == 4)
    {
      int k;
      cin >> k;
      bookshelf.roll_back(k, 0, N);
    }
    cout << bookshelf.all_prod().score - (1001 - M) * rev_cnt << endl;
  }
